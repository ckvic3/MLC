...
class ResNet(nn.Module):
    def __init__(self, block, layers, zero_init_residual=False,
                 ...
                 norm_layer=None,frozen_stages=1):
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        ...
        print("finish load pretrain model")
    return model


def CreateBackbone(arch='resnet50', pretrained=False, progress=True, **kwargs):
    model = None
    if arch == 'resnet18':
        ...
        print("no model named" + arch + "!")
        exit()
    print("backbone is " + arch)
    return model


if __name__ == '__main__':
    torch.cuda.set_device(8)
    backbone = CreateBackbone(pretrained=True)
    print(backbone)
