...
class GroupModel(nn.Module):
    def __init__(self, super_labels, arch: str, block=Bottleneck, layers=[3, 4, 6, 3], zero_init_residual=False,
        ...
        if self.use_group:
            ...
            for i in range(len(super_labels)):
                self.inplanes = 1024  # self.inplanes会在self._make_layer函数中更新，为了避免这个更新带来的影响，每次循环后重新赋值
    ...
    def load_from_pretrain(self, path):
        ...
        # 主分支
        if self.use_head:
            missing_keys, unexpected_keys = self.group_heads[-1].load_state_dict(head_state_dict,
                                                                                                     strict=False)
